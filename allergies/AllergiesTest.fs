...
[<Fact>]
let ``Testing for eggs allergy - not allergic to anything``() = allergicTo 0 Allergen.Eggs |> should equal false

[<Fact>]
let ``Testing for eggs allergy - allergic only to eggs``() = allergicTo 1 Allergen.Eggs |> should equal true
...
[<Fact>]
let ``Testing for eggs allergy - allergic to everything``() = allergicTo 255 Allergen.Eggs |> should equal true

[<Fact>]
let ``Testing for peanuts allergy - not allergic to anything``() = allergicTo 0 Allergen.Peanuts |> should equal false

[<Fact>]
let ``Testing for peanuts allergy - allergic only to peanuts``() = allergicTo 2 Allergen.Peanuts |> should equal true
...
[<Fact>]
let ``Testing for peanuts allergy - allergic to everything``() = allergicTo 255 Allergen.Peanuts |> should equal true
...
[<Fact>]
let ``Testing for tomatoes allergy - not allergic to anything``() = allergicTo 0 Allergen.Tomatoes |> should equal false
...
[<Fact>]
let ``Testing for tomatoes allergy - allergic to everything``() = allergicTo 255 Allergen.Tomatoes |> should equal true
...
[<Fact>]
let ``Testing for pollen allergy - not allergic to anything``() = allergicTo 0 Allergen.Pollen |> should equal false

[<Fact>]
let ``Testing for pollen allergy - allergic only to pollen``() = allergicTo 64 Allergen.Pollen |> should equal true
...
[<Fact>]
let ``Testing for pollen allergy - allergic to everything``() = allergicTo 255 Allergen.Pollen |> should equal true

[<Fact>]
let ``Testing for cats allergy - not allergic to anything``() = allergicTo 0 Allergen.Cats |> should equal false

[<Fact>]
let ``Testing for cats allergy - allergic only to cats``() = allergicTo 128 Allergen.Cats |> should equal true
...
[<Fact>]
let ``Testing for cats allergy - allergic to everything``() = allergicTo 255 Allergen.Cats |> should equal true

[<Fact>]
let ``List - no allergies``() = list 0 |> should be Empty

[<Fact>]
let ``List - just eggs``() = list 1 |> should equal [ Allergen.Eggs ]

[<Fact>]
let ``List - just peanuts``() = list 2 |> should equal [ Allergen.Peanuts ]

[<Fact>]
let ``List - just strawberries``() = list 8 |> should equal [ Allergen.Strawberries ]

[<Fact>]
let ``List - eggs and peanuts``() = list 3 |> should equal [ Allergen.Eggs; Allergen.Peanuts ]

[<Fact>]
let ``List - more than eggs but not peanuts``() = list 5 |> should equal [ Allergen.Eggs; Allergen.Shellfish ]

[<Fact>]
let ``List - lots of stuff``() =
    list 248
    |> should equal [ Allergen.Strawberries; Allergen.Tomatoes; Allergen.Chocolate; Allergen.Pollen; Allergen.Cats ]
