...
[<Fact>]
let ``Empty plaintext results in an empty ciphertext``() = ciphertext "" |> should equal ""

[<Fact>]
let Lowercase() = ciphertext "A" |> should equal "a"

[<Fact>]
let ``Remove spaces``() = ciphertext "  b " |> should equal "b"

[<Fact>]
let ``Remove punctuation``() = ciphertext "@1,%!" |> should equal "1"
